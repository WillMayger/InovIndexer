...
export class Indexer {
  constructor(lessonsObj) {
    ...
    this.resumableLessons = JSON.parse(JSON.stringify(this.lessons));
  ...
  writeResumableJsonObj() {
    console.log('writing json file to "' + this.files.basePath + '/lessons.json" ...');
    this.files.WriteResumableJsonObj(this.lessons);
  }
      ...
      return new Promise((resolve, reject) => {
        ...
        .then(() => {
          let subAsyncProcess = (it) => {
            return new Promise((resolve, reject) => {
              this.files.WriteFile(lesson.downloads[it].url, fullLessonPath + '/' + lesson.downloads[it].name + '.' + lesson.downloads[it].fileType)
              .then(() => {
                this.resumableLessons.levels[i].childlevels[iChild].lessons[iLesson].downloads[it].splice(it, 1);
                resolve();
              })
          ...
          return subPool.start().then(() => {
            this.resumableLessons.levels[i].childlevels[iChild].lessons[iLesson].splice(it, 1);
          });
        })
        .then(() => {
          resolve();
