...
export class Pod {
  ...
  write() {
    ...
    const asyncProcess = (iChild, i, iLesson) => {
      return new Promise((resolve, reject) => {
        let parentFolder = this.lessons[i].name.split(' ').join('_'),
            childFolder = this.lessons[i].childlevels[iChild].name.split(' ').join('_'),
            lesson = this.lessons[i].childlevels[iChild].lessons[iLesson],
