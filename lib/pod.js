    ...
    const asyncProcess = (i, iChild, iLesson) => {
      return new Promise((resolve, reject) => {
        let parentFolder = this.lessons.levels[i].name.split(' ').join('_'),
          ...
          this.horseman.cookies()
          .then((cookies) => {
              return files.setCookies(cookies);
          })
        .then(() => {return files.WriteFolder(parentFolder)})
      ...
      pdfs.map((index, elem) => {
        ...
        switch (pdfItem.text().toLowerCase()) {
          ...
          case 'lesson transcript':
            ...
            break;
          case 'checklist':
            this.lessons.levels[i].childlevels[iChild].lessons[iLesson].lessonchecklist = sortUrl(pdfItem.attr('href'));
            break;
          default:
            break;
        }
